...
async function handleCurrentVideo(langCode, langLabel, silent = false) {
    ...
    // Guard against "Discard changes?" prompts for the entire automation run.
    // YouTube Studio can show this as a native window.confirm OR as a custom DOM
    // dialog whenever we navigate between tabs or change a dropdown with unsaved edits.
    ...
    try {
        ...
        if (method === 'new') {
            ...
            log('Step 2 — Navigate to Subtitles (old method)')
...
async function stepAudience(channelLangLabel) {
    ...
    // When the channel is set to "Not for kids" at channel level, YouTube Studio
    // may not show the per-video radio at all — so this is optional (catch → null).
    ...
    if (radio) {
        // Check both the ARIA attribute and the Polymer .checked property so we never
        // accidentally toggle a radio that is already selected (which would mark the
        // video as "Made for kids" and hide the Subtitles sidebar item).
        const radioAlreadyChecked = radio.getAttribute('aria-checked') === 'true' || radio.checked === true
        ...
        log('Audience radio not shown — channel-level "Not for kids" setting covers it')
    }

    // If the radio was just changed, save it immediately before opening the language
    // dropdowns. Changing a dropdown while there are unsaved edits triggers YouTube
    // Studio's "Discard changes?" confirm dialog — saving first prevents that.
    if (radioChanged) {
        const saveBtn = await waitFor(
            () => Array.from(document.querySelectorAll('ytcp-button, button'))
                                .find(b => /^save$/i.test(b.textContent.trim()) && isVisible(b) && !b.disabled),
            8000,
            'Save button (audience)'
        )
        saveBtn.click()
        log('Audience save clicked')
        await sleep(1500)
    }
    ...
    // Save if the Save button became enabled after language changes.
    // Use waitFor (not an immediate find) so we catch the button even if it takes
    // a moment to become enabled — missing it means navigating to Subtitles with
    // unsaved changes, which triggers YouTube Studio's "Discard changes?" dialog.
    const saveBtn = await waitFor(
        () => Array.from(document.querySelectorAll('ytcp-button, button'))
            .find(b => /^save$/i.test(b.textContent.trim()) && isVisible(b) && !b.disabled),
        2500,
        'Save button after language change'
    ).catch(() => null)
    if (saveBtn) {
        saveBtn.click()
        log('Language save clicked')
        await sleep(1500)
    }
}
...
async function stepGoToSubtitles() {
    const link = await waitFor(
        ...
        throw new Error(
            'Subtitles option not found in the sidebar. ' +
            'The video may be restricted, still processing, or the Studio UI did not load fully.'
        )
    })
    link.click()
    await sleep(500)
...
/**
 * Detects whether this video uses the old "Subtitles" tab or the new "Languages"
 * tab. Returns 'old' or 'new'. Waits up to 10 s for either to appear.
 */
async function detectTranslationMethod() {
    return waitFor(
        () => {
            if (findAnchorByText('Languages')) return 'new'
            if (findAnchorByText('Subtitles')) return 'old'
            return null
        },
        10000,
        '"Subtitles" or "Languages" sidebar tab'
    )
}

// ── New method: Languages tab ─────────────────────────────────────────────────

/**
 * Full automation for the new YouTube Studio UI where the sidebar shows a
 * "Languages" tab instead of "Subtitles".
 *
 * Flow:
...
async function stepNewMethod(langLabel, channelLangLabel) {
    ...
    if (hasLanguageAlreadyNew(langLabel)) {
        log(`"${langLabel}" already published — skipping`)
        return { skipped: true }
    }

    // 3. Click "Edit subtitles" to open the original-language subtitle editor
    const editBtn = await waitFor(
        () => {
            const btn = document.querySelector('button[aria-label="Edit subtitles"]')
    ...
    // Wait to return to Languages list
    await waitFor(
        () => document.querySelector('button[aria-label="Add language"]'),
        15000,
        '"Add language" button after publish'
    )
    await sleep(500)

    // 5. Click "Add language"
    const addLangBtn = document.querySelector('button[aria-label="Add language"]')
    addLangBtn.click()
    await sleep(700)

    // 6. Select the target language from the dropdown list
    ...
    langOption.click()
    log(`Selected language "${langLabel}"`)
    await sleep(800)

    // 7. Click "Add" on the "Manual subtitles" row
    const manualRow = await waitFor(
        () => document.querySelector('tr.manual-subtitles-row'),
        8000,
        '"Manual subtitles" row'
    )
    const addBtn = await waitFor(
        () => {
            const btn = manualRow.querySelector('button[aria-label="Add"]')
            return (btn && !btn.disabled && btn.getAttribute('aria-disabled') !== 'true') ? btn : null
        },
        5000,
        '"Add" button on Manual subtitles row'
    )
    addBtn.click()
    log('Clicked "Add" on Manual subtitles row')
    await sleep(1500)

    // Handle title/description translation dialog if it appears
    await maybeHandleTitleDescriptionDialog()

    // 8. Click "Auto-translate"
    const autoTranslate = await waitForAutoTranslateEnabled(10000)
    if (!autoTranslate) return { retryNeeded: true }
    ...
    await waitFor(
        () => hasLanguageAlreadyNew(langLabel),
        15000,
        `"${langLabel}" row with "1 Published" in Languages list`
    ).catch(() => log('Note: could not verify "1 Published" status — proceeding'))

    return {}
}

/**
 * Returns true if `langLabel` already has a row with "1 Published" in the
 * Subtitles column of the new Languages-tab translations list.
 */
function hasLanguageAlreadyNew(langLabel) {
    return Array.from(document.querySelectorAll('ytgn-video-translation-row'))
        .some(row => {
...
/**
 * Installs a guard for the entire automation run that auto-dismisses any
 * "Discard changes?" prompt, whether it appears as a native window.confirm
 * or as a custom YouTube Studio DOM dialog.
 * Returns a stop() function — call it in a finally block.
 */
function startDiscardGuard() {
    const origConfirm = window.confirm
    window.confirm = (msg) => (/discard/i.test(msg ?? '') ? true : origConfirm.call(window, msg))

    // Modal dialogs use position:fixed which makes offsetParent null — isVisible()
    // returns false for them. Use a lighter check that only tests computed style.
    const isShown = el => {
        if (!el) return false
    ...
    const observer = new MutationObserver(() => {
        const dialogs = Array.from(document.querySelectorAll(
            'tp-yt-paper-dialog, ytcp-dialog, [role="dialog"], ytcp-confirmation-dialog'
        )).filter(d => /discard/i.test(d.textContent) && isShown(d))

        for (const dialog of dialogs) {
            // Click the "Discard" button — avoid "Cancel" / "Keep editing".
            const btn = Array.from(dialog.querySelectorAll('ytcp-button, button'))
                .find(b => /^discard/i.test(b.textContent.trim()) && isShown(b))
            if (btn) {
                btn.click()
                log('Auto-dismissed "Discard changes?" dialog')
            }
        }
    })
